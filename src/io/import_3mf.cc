...
Geometry *import_3mf(const std::string& filename, const Location& loc)
{
  ...
  if (first_mesh == 0) {
    ...
    children.push_back(std::make_pair(std::shared_ptr<AbstractNode>(),  std::shared_ptr<const Geometry>(first_mesh)));
    for (polysets_t::iterator it = meshes.begin(); it != meshes.end(); ++it) {
      children.push_back(std::make_pair(std::shared_ptr<AbstractNode>(),  std::shared_ptr<const Geometry>(*it)));
    }
