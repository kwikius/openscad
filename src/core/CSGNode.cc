...
std::shared_ptr<CSGNode> CSGOperation::createCSGNode(OpenSCADOperator type, std::shared_ptr<CSGNode> left, std::shared_ptr<CSGNode> right)
{
  ...
  return std::shared_ptr<CSGNode>(new CSGOperation(type, left, right), CSGOperationDeleter());
}

CSGLeaf::CSGLeaf(const std::shared_ptr<const Geometry>& geom, const Transform3d& matrix, const Color4f& color, const std::string& label, const int index)
...
CSGOperation::CSGOperation(OpenSCADOperator type, std::shared_ptr<CSGNode> left, std::shared_ptr<CSGNode> right)
  : type(type)
{
